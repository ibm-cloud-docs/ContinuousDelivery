...
Everything that changes the [baseline](#cd-devsecops-inventory) must be traced by way of a change request. These changes include updates to the existing code level, changes to the configuration, and updates of the worker nodes. Collection of [peer review compliance](#cd-devsecops-peer-review) data is based on the data that is accessible in the inventory, the evidence locker, and the incident issue repo.

This step creates the change request by attaching the available compliance data based on the [promotion pull request](#cd-devsecops-pr-pipeline) fields. [Deployment readiness](#cd-devsecops-change-mgmt) is calculated based on the available evidence in the collected compliance status.
...
* [Pipeline trigger webhook](#cd-devsecops-webhook-payloads)

For more information about how to access these variables, see [Custom scripts](#cd-devsecops-custom-scripts).
...
For more information about inventory conclude, see [Inventory](#cd-devsecops-inventory).
