...
| Simplified Cloud Foundry org and space shell | Provides a test job with options to run tests in different regions. | **Pipeline image version**: Runs in a container by using a built-in docker image that provides various built-in commands. To adopt newer versions of those commands, use a newer image version.  \n  \n **IBM Cloud region**: The environment to deploy to application to.  \n  \n **Shell script**: Shell command to run whenever the job runs. In the script field, enter a script or reference scripts that are stored in your project’s source control.  \n  \n **Enable test report**: Not used.|
