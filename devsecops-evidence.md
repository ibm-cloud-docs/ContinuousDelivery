...
Evidence is collected from the steps in each stage by using the same process that is used to [create evidence](#cd-devsecops-evidence-creation). The following image shows the current steps in the reference pipelines that produce evidence.
...
For more information about {{site.data.keyword.cos_short}} buckets, see [{{site.data.keyword.cos_full_notm}} buckets as evidence locker](/docs/ContinuousDelivery?topic=ContinuousDelivery-cd-devsecops-cos-bucket-evidence).
