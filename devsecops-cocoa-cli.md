...
<!--INTERNAL ONLY CONTENT: this prereq requirement goes to internal only page. this link is also used other places in this topic and needs to be updated. There are also a few mentions of servicenow API in the commands section-->

You must have a TaaS Artifactory API Key to download the CLI. To get one, go to [this guide](https://taas.cloud.ibm.com/guides/create-apikey-in-artifactory.md){: external}.

### Getting the `npm` package
{: #devsecops-cli-npm}

Make sure that `npm` is configured to be able to fetch code from [TaaS Artifactory](https://na.artifactory.swg-devops.com/){: external}.
...
Where `<BASE64_PASSWORD>` is your `base64` encoded [API key](https://na.artifactory.swg-devops.com/ui/admin/artifactory/user_profile){: external}, `<USERNAME>` is your IBM internal email.

<!-- I assume "IBM internal email needs to be changed here-->
...
In this case replace `<EMAIL>` with your IBM email address, and provide your [API key](https://taas.cloud.ibm.com/guides/create-apikey-in-artifactory.md){: external} as password.

Install the package from our [`npm` registry](https://na.artifactory.swg-devops.com/){: external}:

```sh
$ npm install -g @cocoa/cli
```
{: codeblock}

### Getting the `docker` image
{: #devsecops-cli-get-docker}

Log in to our Docker registry:

```sh
$ docker login wcp-compliance-automation-team-docker-local.artifactory.swg-devops.com

> Username: enter your IBM email here

> Password: enter your TaaS Artifactory API key here
```
{: codeblock}

If You have logged in successfully, pull the image from artifactory:

```sh
$ docker pull wcp-compliance-automation-team-docker-local.artifactory.swg-devops.com/ibm-compliance-automation:<version>
```
{: codeblock}
...
```sh
$ docker run -it wcp-compliance-automation-team-docker-local.artifactory.swg-devops.com/ibm-compliance-automation:<version>
```
{: codeblock}

Start using the CLI inside the image:
...
For each version, the binaries are named after this pattern: `cocoa-<platform>-<version>`.

On linux/macOS You can download the binary with `curl`:

```sh
$ curl -u <ibm-email-address>:<taas-artifactory-api-key> -O "https://na.artifactory.swg-devops.com/artifactory/wcp-compliance-automation-team-generic-local/cocoa-<platform>-<version>"
```
{: codeblock}
...
```sh
$ chmod +x ./cocoa-<platform>-<version>
```
{: codeblock}

Run the CLI via executable:

```sh
# macOS
$ ./cocoa-macos-<version> --help

# linux
$ ./cocoa-linux-<version> --help
```
{: codeblock}
...
The CoCoA CLI uses a multipart structure on the command line that must be specified in this order:
