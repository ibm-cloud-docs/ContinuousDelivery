...
{{site.data.keyword.contdelivery_short}} fournit les droits nécessaires pour gérer les données dans une organisation Cloud Foundry. Votre entreprise peut avoir des règles qui limitent ces droits. Si vous ne disposez pas des droits nécessaires, contactez l'administrateur de votre organisation.
...
**Astuce** : avant de supprimer vos données dans {{site.data.keyword.webide}}, pensez à exporter votre travail. Une fois supprimés, vos espaces de travail seront retirés de {{site.data.keyword.contdelivery_short}} et tous les fichiers seront supprimés.
...
1. Cliquez sur l'icône **Paramètres** <img class="inline" src="images/webide_settings_icon_light_small.png"  alt="Icône Paramètres"> dans la barre de navigation latérale de gauche.
...
**Important** : avant de supprimer votre compte Git, supprimez vos données personnelles des autres projets. Après avoir supprimé votre compte Git, il peut être difficile ou impossible de trouver tous les projets auxquels vous avez contribué. 
...
###Modification de la propriété d'un pipeline

Vous pouvez transférer la propriété d'un pipeline qui vous appartient à un autre utilisateur. Le nouvel utilisateur doit réaliser l'opération de transfert. Un utilisateur peut devenir propriétaire d'un pipeline existant, mais il ne peut pas en attribuer la propriété à un autre utilisateur. Vous devez avoir des droits d'administrateur pour devenir propriétaire d'un pipeline.

Pour devenir propriétaire d'un pipeline, vous devez suivre les étapes suivantes :

1. Connectez-vous à IBM Cloud.
1. Cliquez sur le menu pour accéder aux options de configuration. 

  ![Menu Configuration](images/pipeline_ownership.png)

1. Cliquez sur **Modifier le propriétaire du pipeline**. 
1. Sélectionnez **ME RENDRE PROPRIETAIRE**.
...
Vous ne pouvez pas supprimer toutes les chaînes d'outils d'une organisation en même temps. Vous devez supprimer les chaînes d'outils une par une.

**Important** : les chaînes d'outils sont délimitées par région IBM Cloud et organisation Cloud Foundry. Assurez-vous de sélectionner chaque région et organisation au sein de la région pour supprimer toutes les chaînes d'outils que vous avez créées. 
