...
{{site.data.keyword.contdelivery_short}} fornisce le autorizzazioni appropriate per gestire i dati all'interno di una organizzazione Cloud Foundry. La tua azienda potrebbe avere delle politiche che limitano queste autorizzazioni. Se non disponi delle le autorizzazioni appropriate, contatta l'amministratore della tua organizzazione.
...
**Suggerimento**: prima di eliminare i tuoi dati {{site.data.keyword.webide}}, sarebbe opportuno esportare il tuo lavoro. Dopo che li hai eliminati, i tuoi spazi di lavoro vengono rimossi da {{site.data.keyword.contdelivery_short}} e vengono eliminati tutti i file.
...
**Importante**: prima di eliminare il tuo account Git, elimina i tuoi dati personali dagli altri progetti. Dopo che hai eliminato il tuo account Git, potrebbe essere difficile, se non impossibile, trovare tutti i progetti a cui hai contribuito.
...
**Suggerimento**: dopo che il tuo account è stato eliminato, del contenuto continuerà a esistere. Questo contenuto viene assegnato a un Ghost User a livello di sistema. Per ulteriori informazioni sull'eliminazione di un account {{site.data.keyword.gitrepos}}, consulta [Deleting a user account ![Icona link esterno](../../icons/launch-glyph.svg "Icona link esterno")](https://git.ng.bluemix.net/profile/account/delete_account#associated-records){:new_window}.
...
## Modifica ed eliminazione dei dati delle pipeline Continuous Delivery
...
**Suggerimento**: una pipeline non può esistere al di fuori di una toolchain. Se elimini una toolchain, vengono eliminate anche tutte le pipeline ad essa associate. Se intendi eliminare un'intera toolchain, non hai bisogno di eliminare ciascuna pipeline singolarmente. Passa invece direttamente alla sezione "Modifica ed eliminazione delle toolchain e delle integrazioni dello strumento" e attieniti alla procedura per eliminare una toolchain.

###Modifica ed eliminazione delle fasi della pipeline

Le fasi della pipeline possono includere dati personali quali le credenziali sotto forma di proprietà dell'ambiente. Le fasi possono anche includere degli script all'interno di lavori che vuoi modificare o eliminare. Utilizza le azioni Configure Stage o Delete Stage per modificare o eliminare una fase.
...
###Modifica della proprietà della pipeline

Puoi trasferire la proprietà di una pipeline che ti appartiene a un altro utente; il nuovo proprietario deve eseguire l'operazione di trasferimento. Un utente può assumere la proprietà di una pipeline esistente ma non può assegnare la proprietà a un altro utente. Devi disporre dei diritti di amministratore (ADMIN) per assumere la proprietà di una pipeline.

Per assumere la proprietà di una pipeline, un utente deve completare la seguente procedura:

1. Esegui l'accesso a IBM Cloud.
1. Fai clic sul menu per accedere alle opzioni di configurazione 

  ![Menu Configuration](images/pipeline_ownership.png)

1. Fai clic su **Modify Pipeline Owner**. 
1. Seleziona **MAKE ME OWNER**.
...
Non puoi eliminare tutte le toolchain in un'organizzazione contemporaneamente. Devi eliminare ciascuna toolchain, una alla volta.

**Importante**: le toolchain sono organizzate per regione IBM Cloud e organizzazione Cloud Foundry. Assicurati di selezionare ogni regione e organizzazione all'interno della regione per eliminare ogni toolchain che hai creato.
