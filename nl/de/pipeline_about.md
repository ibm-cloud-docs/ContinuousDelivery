...
**Wichtig**: Die Jobs in einer Stage können einander keine Artefakte übergeben. Da Artefakte zwischen den Stages nicht übergeben werden können, müssen Sie von einer Stage für die Bereitstellung eine separate Build-Stage besitzen, wenn Ihre Stage für die Bereitstellung die Build-Stage-Artefakte verwenden soll.
...
Beispiele für Bod- und Bereitstellungsscripts finden Sie in [https://github.com/open-toolchain/commons](https://github.com/open-toolchain/commons).
...
Build-Jobs rufen automatisch den Inhalt aus dem aktuellen Ordner ab, in dem das Benutzerscript ausgeführt wird. Wenn Sie nicht den gesamten Git-Repo-Inhalt für eine spätere Bereitstellung benötigen, sollten Sie ein explizites Ausgabeverzeichnis konfigurieren, damit die relevanten Artefakte dort kopiert oder erstellt werden. Job-Scripts werden im Buildergebnis (Ausgabeverzeichnis) ausgeführt.

Bereitstellungsjobs, die Bereitstellungen auf Cloud Foundry ausführen, müssen die Organisation und den Bereich angeben, wo die Artefakte bereitgestellt werden. Wenn zur Ausführung Ihrer App weitere Services notwendig sind, müssen Sie sie in der Datei `manifest.yml` angeben.

Bereitstellungsjobs, die den IBM Cloud-Container-Service auf einem Kubernetes-Cluster bereitstellen, benötigen eine Dockerfile und optional ein Helm-Diagramm.  

Das Job-Script wird ausgeführt, sobald sich der Job in die Zielumgebung angemeldet hat (damit Sie die Befehle `cf push` oder `kubectl` im Script ausführen können).
