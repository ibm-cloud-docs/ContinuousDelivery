...
**Wichtig**: Wenn Sie alle Ihre persönlichen Daten aus {{site.data.keyword.contdelivery_short}} löschen möchten, ist die Reihenfolge wichtig, in der Sie diese Daten löschen. Löschen Sie zuerst Ihre {{site.data.keyword.webide}}-Arbeitsbereiche. Dann löschen Sie Ihre {{site.data.keyword.gitrepos}}-Daten. Löschen Sie anschließend Ihr {{site.data.keyword.gitrepos}}-Konto. Löschen Sie zuletzt Ihre Delivery Pipelines, Toolintegrationen und Toolchains. 
...
**Tipp**: Bevor Sie Ihre {{site.data.keyword.webide}}-Daten löschen, sollten Sie Ihre Arbeit exportieren. Wenn Sie Ihre Arbeitsbereiche gelöscht haben, werden Sie aus {{site.data.keyword.contdelivery_short}} entfernt und alle Dateien sind gelöscht.
...
**Wichtig**: Löschen Sie erst Ihre persönlichen Daten aus anderen Projekten, bevor Sie Ihr Git-Konto löschen. Wenn Sie Ihr Git-Konto gelöscht haben, kann es schwierig oder unmöglich sein, alle Projekte zu finden, an denen Sie mitgewirkt haben. 
...
**Tipp**: Nachdem Ihr Konto gelöscht wurde, bleiben einige Inhalte erhalten. Dieser Inhalt wird einem systemweiten Ghost-Benutzer zugewiesen. Weitere Informationen zum Löschen eines {{site.data.keyword.gitrepos}}-Kontos finden Sie in [Benutzerkonten löschen ![Symbol für externen Link](../../icons/launch-glyph.svg "Symbol für externen Link")](https://git.ng.bluemix.net/profile/account/delete_account#associated-records){:new_window}.

**Wichtig**: {{site.data.keyword.gitrepos}} verwendet einen Single-Sign-On, der automatisch ein Git-Konto für Ihr IBM Cloud-Konto erstellt, wenn Sie das erste Mal auf die Toolintegration zugreifen. Wenn Sie Ihr Konto gelöscht haben, greifen Sie nicht auf {{site.data.keyword.gitrepos}} zu. Denn wenn Sie erneut auf {{site.data.keyword.gitrepos}} zugreifen, wird automatisch ein neues Konto erstellt, das Sie dann wieder löschen müssen.

## Continuous-Delivery-Pipeline-Daten ändern und löschen
...
###Pipeline-Stages ändern und löschen

Pipeline-Stages können persönliche Daten wie Berechtigungsnachweise in Form von Umgebungseigenschaften enthalten. Stages enthalten möglicherweise auch Scripts in Jobs, die Sie ändern oder löschen möchten. Verwenden Sie die Aktionen zum Konfigurieren oder zum Löschen von Stages, um eine Stage zu ändern oder zu löschen. 
...
###Pipeline-Eigentumsrechte ändern

Sie können die Eigentumsrechte Ihrer Pipeline an einen anderen Benutzer übertragen. Dabei muss der neue Besitzer die Übertragung durchführen. Ein Benutzer kann das Eigentumsrecht einer Pipeline übernehmen, aber er kann einem anderen Benutzer keine Eigentumsrechte zuweisen. Sie müssen über ADMIN-Rechte verfügen, um das Eigentumsrecht einer Pipeline zu erhalten.

Führen Sie die folgenden Schritte aus, um das Eigentumsrecht einer Pipeline zu erhalten:

1. Melden Sie sich bei IBM Cloud an.
1. Klicken Sie auf das Menü, um auf die Konfigurationsoptionen zuzugreifen. 

  ![Konfigurationsmenü](images/pipeline_ownership.png)

1. Klicken Sie auf **Pipeline-Eigentümer ändern**. 
1. Wählen Sie **MICH ZUM EIGENTÜMER MACHEN** aus.
...
Sie können nicht alle Toolchains in einer Organisation gleichzeitig löschen. Sie müssen jede Toolchain einzeln löschen.

**Wichtig**: Toolchains werden von der IBM Cloud-Region und der Cloud Foundry-Organisation verwaltet. Stellen Sie sicher, dass Sie jede Region und Organisation in der Region auswählen, um jede von Ihnen erstellte Toolchain zu löschen.
