...
ここで、`user` は GitLab ユーザー名、`XXXXXXX` はアクセス・トークン、[`group` ![外部リンク・アイコン](../../icons/launch-glyph.svg "外部リンク・アイコン")](https://git.ng.bluemix.net/help/user/group/index.md){:new_window} はリポジトリーが保管されているグループ、`node-hello-world` はリポジトリー名です。
