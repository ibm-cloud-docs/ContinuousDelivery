...
* Cloud Foundry 組織内のツールチェーンに対するユーザー・アクセス権限は、リソース・グループ内のツールチェーンに対するユーザー・アクセス権限と異なる方法で管理されます。Cloud Foundry 内のツールチェーンに対するユーザー・アクセス権限の管理について詳しくは、[Cloud Foundry の組織内のツールチェーンへのアクセスの管理](/docs/services/ContinuousDelivery/toolchains_using.html#managing_access_orgs){: new_window}を参照してください。
...
|:-----------------|:-----------------|:-----------------|
| ビューアー、オペレーター | {{site.data.keyword.contdelivery_short}} サービスのインスタンスを表示します。| <ul><li>{{site.data.keyword.contdelivery_short}} サービス・インスタンスをクリックして、ダッシュボードを開く。</li>|
