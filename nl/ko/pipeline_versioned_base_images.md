 ...
 * **버전 2.1**: 버전 2.1의 컨텐츠를 보려면 실행 중인 이미지에서 `default_versions.sh`를 입력하십시오. 이 이미지는 다음 도구를 포함합니다.
...
```
 ...
 버전 2.0 이후 이미지에는 더 이상 `grunt` 또는 `python`이 포함되지 않습니다. 필요한 경우 `grunt` 또는 `python`을 설치하도록 선택할 수 있습니다.
 {: tip}
