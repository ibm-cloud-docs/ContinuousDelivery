...
{{site.data.keyword.contdelivery_short}}는 Cloud Foundry 조직 내에서 데이터를 관리하기 위한 적절한 권한을 제공합니다. 회사에는 이러한 권한을 제한하는 정책이 있을 수 있습니다. 적절한 권한이 없는 경우 조직 관리자에게 문의하십시오. 
...
IBM Cloud에서 작업하려면 사용자 이름과 비밀번호를 사용하여 로그인해야 합니다. 로그인하는 경우 IBM Cloud는 사용자 신임 정보를 사용하여 하나 이상의 IBM Cloud 계정과 연관됩니다. Cloud Foundry 조직 및 {{site.data.keyword.contdelivery_short}} 오브젝트와 같은 리소스를 작성하는 경우 리소스는 IBM Cloud 계정과 연관됩니다. 
...
**중요**: {{site.data.keyword.contdelivery_short}}에서 모든 개인 데이터를 삭제하려는 경우 데이터를 삭제하는 순서가 중요합니다. 먼저, 모든 {{site.data.keyword.webide}} 작업공간을 삭제하십시오. 그런 다음, {{site.data.keyword.gitrepos}} 데이터를 삭제한 후 {{site.data.keyword.gitrepos}} 계정을 삭제하십시오. 마지막으로 Delivery Pipeline, 도구 통합 및 도구 체인을 삭제하십시오. 
...
**팁**: {{site.data.keyword.webide}} 데이터를 삭제하기 전에 작업을 내보내려고 할 수 있습니다. 작업공간을 삭제하고 나면 {{site.data.keyword.contdelivery_short}}에서 제거되고 모든 파일이 삭제됩니다. 
...
**중요**: {{site.data.keyword.webide}}는 싱글 사인온 메커니즘을 사용합니다. 이 도구 통합에 처음 액세스하는 경우 IBM Cloud 계정에 대해 숨겨진 해당 {{site.data.keyword.webide}} 계정이 작성됩니다. 모든 작업공간을 삭제한 후에는 {{site.data.keyword.webide}}에 액세스하지 마십시오. 다시 {{site.data.keyword.webide}}에 액세스하는 경우 삭제해야 하는 새 계정이 자동으로 작성됩니다. 
...
**중요**: Git 계정을 삭제하기 전에 다른 프로젝트의 개인 데이터를 삭제하십시오. Git 계정을 삭제하고 나면 사용자가 참가한 모든 프로젝트를 찾기가 어렵거나 불가능할 수 있습니다. 
...
**팁**: 계정이 삭제된 후에 일부 컨텐츠는 유지됩니다. 이 컨텐츠는 시스템 범위 고스트(Ghost) 사용자에게 지정됩니다. {{site.data.keyword.gitrepos}} 계정 삭제에 대한 자세한 정보는 [사용자 계정 삭제 ![외부 링크 아이콘](../../icons/launch-glyph.svg "외부 링크 아이콘")](https://git.ng.bluemix.net/profile/account/delete_account#associated-records){:new_window}을 참조하십시오.

**중요**: {{site.data.keyword.gitrepos}}는 도구 통합에 처음 액세스할 때 IBM Cloud 계정에 대한 해당 Git 계정을 자동으로 작성하는 싱글 사인온 메커니즘을 사용합니다. 계정을 삭제하고 나면 {{site.data.keyword.gitrepos}}에 액세스하지 마십시오. 다시 {{site.data.keyword.gitrepos}}에 액세스하는 경우 삭제해야 하는 새 계정이 자동으로 작성됩니다. 

## Continuous Delivery 파이프라인 데이터 수정 및 삭제
...
**팁**: 파이프라인은 도구 체인 외부에 존재할 수 없습니다. 도구 체인을 삭제하는 경우 해당 도구 체인과 연관되는 모든 파이프라인도 삭제됩니다. 전체 도구 체인을 삭제할 계획이면 각 파이프라인을 개별적으로 삭제할 필요가 없습니다. 대신 "도구 체인 및 도구 통합 수정 및 삭제" 섹션으로 건너뛰어 해당 단계에 따라 도구 체인을 삭제하십시오. 

###파이프라인 단계 수정 및 삭제

파이프라인 단계는 신임 정보와 같은 개인 데이터를 환경 특성 양식에 포함할 수 있습니다. 단계는 수정하거나 삭제하려는 작업 안에 스크립트를 포함할 수도 있습니다. 단계 구성 또는 단계 삭제 조치를 사용하여 단계를 수정하거나 삭제하십시오. 
...
사용자가 소유하고 있는 파이프라인의 소유권을 다른 사용자에게 이전할 수 있습니다. 새 소유자가 이전 오퍼레이션을 수행해야 합니다. 사용자는 기존 파이프라인의 소유권을 가질 수 있지만 다른 사용자에게 소유권을 지정할 수는 없습니다. 파이프라인의 소유권을 지정하려면 관리자 권한이 있어야 합니다. 

파이프라인의 소유권을 가지려면 사용자는 다음 단계를 완료해야 합니다. 

1. IBM Cloud에 로그인하십시오.
1. 메뉴를 클릭하여 구성 옵션에 액세스하십시오.  

  ![구성 메뉴](images/pipeline_ownership.png)

1. **파이프라인 소유자 수정**을 클릭하십시오. 
1. **나를 소유자로 설정**을 선택하십시오.
...
동시에 조직 내에서 모든 도구 체인을 삭제할 수는 없습니다. 각 도구 체인을 한 번에 하나씩 삭제해야 합니다. 

**중요**: 도구 체인은 IBM Cloud 지역 및 Cloud Foundry 조직에 의해 범위가 지정됩니다. 작성한 모든 도구 체인을 삭제하려면 해당 지역 내에서 각 지역 및 조직을 선택해야 합니다. 
