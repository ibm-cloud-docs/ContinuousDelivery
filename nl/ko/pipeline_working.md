...
1. 다른 조직에 파이프라인을 작성하려는 경우 해당 기본 조직을 변경하십시오. 사용자가 속해 있는 조직에 파이프라인을 작성할 수 있습니다. 
1. 다른 영역에 파이프라인을 작성하려는 경우 해당 기본 영역을 변경하십시오. 사용자 조직에 있는 영역에 파이프라인을 작성할 수 있습니다. 
...
1. 컴포지트 파이프라인을 포함하는 도구 체인에 파이프라인을 작성한 경우 새 파이프라인이 컴포지트 파이프라인에 추가됩니다. 새 파이프라인의 배치 태스크가 포함되도록 배치 플랜을 수정하십시오. [Delivery Pipeline 작성 태스크](/docs/services/ContinuousDelivery/pipeline_deployment_plan.html#tasks_pipelineCD){: new_window}를 참조하십시오.
