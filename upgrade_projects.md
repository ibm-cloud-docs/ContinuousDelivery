...
{{site.data.keyword.jazzhub}} is evolving into {{site.data.keyword.contdelivery_full}}. As part of that change, projects are upgraded to toolchains.
...
- [My JazzHub project is associated with the UK region, but my toolchain is in the US South region. How will this work?](#faq_region)
- [What happens to my work items and dashboards in Track &amp; Plan when I upgrade?](#faq_tp)
- [What happens to my code repo when I upgrade?](#faq_repo)
- [What happens to my build definitions in my project when I upgrade to a toolchain?](#faq_build)
- [I need to create an organization for my project that is upgraded to a toolchain. I understand that I need to add a credit card to my account before I can create an organization. Will my credit card be charged?](#faq_charges)
- [I can't find or access my toolchain. What can I do?](#faq_find)
...
- Make sure that your {{site.data.keyword.jazzhub_short}} project owner is correct. The toolchain that is created from your project is part of that owner's {{site.data.keyword.Bluemix_notm}} organization.
...
Projects at hub.jazz.net and toolchains are both hosted in the US South region. If your project was configured to deploy apps to a different region, it still deploys apps to that region after it is upgraded to a toolchain.
{: tip}
...
When you start the upgrade, the pipeline stages in your project are locked. You can't run or modify them. If you revert the upgrade by deleting the toolchain, the pipeline is unlocked.
...
1. To start the upgrade process, on the banner message, click **upgrade now**. The "Project upgrade toolchain" page opens.
   ...
   For an overview of the upgrade process, read the description on that page. The toolchain includes a new pipeline that contains the same stages and jobs as the project's pipeline. In addition, the toolchain contains a pointer to the Eclipse Orion {{site.data.keyword.webide}} that runs in {{site.data.keyword.contdelivery_short}}.

   In this example, because the project uses a public repo on github.com, the toolchain is connected to the same GitHub repo. If your project uses a Git repo that is hosted on JazzHub, the contents of that repo are cloned to a new repo in {{site.data.keyword.gitrepos}}, which is part of {{site.data.keyword.contdelivery_short}}.
...
4. If you are using {{site.data.keyword.gitrepos}}, authenticate by using a personal access token or an SSH key. For more information about SSH keys, see [Creating a personal access token or SSH key for authentication](/docs/services/ContinuousDelivery/git_working.html#git_authentication). To authenticate from an external Git client through https, follow these steps:
    ...
    4. To authenticate with {{site.data.keyword.gitrepos}} from an external Git client through https, use your user name and your personal access token.
...
5. When your toolchain is set up and you start to use it, consider taking all or any of these steps to ensure that no one uses your project:
...
### My JazzHub project is associated with the UK region, but my toolchain is in the US South region. How does this work?
{: #faq_region}

Projects at hub.jazz.net and toolchains are both hosted in the US South region. If your project is configured to deploy apps to a different region, such as the UK region, it still deploys apps to that region after it is upgraded to a toolchain. Therefore, nothing is really changing regarding where the data is hosted. Toolchains will be available in more regions in the future.

### What happens to my work items and dashboards in Track &amp; Plan when I upgrade?
{: #faq_tp}
...
Additionally, to purchase Build Automation and Source Code Management, [Rational Team Concert on Cloud ![External link icon](../../icons/launch-glyph.svg "External link icon")](https://www.ibm.com/us-en/marketplace/change-and-configuration-management/purchase#product-header-top){: new_window} is an option.

### What happens to my code repo when I upgrade?
{: #faq_repo}
...
{: caption="Table 1. Project repos that are mapped to toolchain repos" caption-side="top"}


### What happens to my build definitions in my project when I upgrade to a toolchain?
{: #faq_build}
...
If you're using Jazz SCM as a source repo and using Delivery Pipeline to build your code, the source in Jazz SCM is automatically moved to a Git repo. Your Delivery Pipeline configuration remains the same except that it consumes the source from the Git repo instead of the source from Jazz SCM.
...
### I can't find or access my toolchain. What can I do?
{: #faq_find}
...
These issues can occur if you came from a non-US {{site.data.keyword.Bluemix_notm}} organization and didn't explicitly expand your organization to the US South region before you upgraded. You can confirm this scenario in two ways:

   * When you open the toolchain URL, check the {{site.data.keyword.Bluemix_notm}} header. Most likely, you'll see your organization name and no space is indicated.
